...
namespace DI
{
    public class DIContainer
    {
        ...
        public T Resolve<T>(string tag = null)
        {
            ...
            {
                throw new Exception($"Cycling dependency for tag {tag} and type {key.Item2.FullName}");
            }
            ...
            try
            {
                if (_registrations.TryGetValue(key, out var registration))
                {
                    ...
                    {
                        if (registration.Instanse == null && registration.Factory != null)
                        {
                            registration.Instanse = registration.Factory(this);
                        }

                        return (T)registration.Instanse;
                    }
